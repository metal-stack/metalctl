...
```
  -h, --help               help for list
      ...
      --order strings      order by (comma separated) column(s), sort direction can be changed by appending :asc or :desc behind the column identifier. possible values: project|liveliness|age|id|size|image|partition|when|event
