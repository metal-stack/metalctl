...
func (m MetalMachineTablePrinter) Print(data []*models.V1MachineResponse) {
	...
	for _, machine := range data {
		...
		if machine.State != nil && machine.State.MetalHammerVersion != nil && strings.HasPrefix(lastEventEmoji, "Waiting") {
			lastEventEmoji = lastEventEmoji + " " + *machine.State.MetalHammerVersion
		}
