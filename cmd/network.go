...
func newNetworkCmd(c *config) *cobra.Command {
	...
	cmdsConfig := &genericcli.CmdsConfig[*models.V1NetworkCreateRequest, *models.V1NetworkUpdateRequest, *models.V1NetworkResponse]{
		...
		CreateCmdMutateFn: func(cmd *cobra.Command) {
			...
			cmd.Flags().StringSlice("labels", []string{}, "add initial annotation, must be in the form of key=value, can be given multiple times to add multiple annotations, e.g. --annotation key=value --annotation foo=bar")
